...
| B9 | Quality Gates & CI | 🚧 In Progress | Configure PHPUnit, PHPStan lvl 8, PHPCS, PHP CS Fixer, composer audit, CI workflows, and coverage thresholds. Bootstrap GitHub Actions workflow running the full suite. | Core features merged; tooling requirements reviewed. | CI green; coverage ≥ 80% lines / 75% branches; audit clean or waivers documented. | Primary: QA Lead; Escalation: Engineering Manager. | GitHub Actions Workflow `CI` provisioniert (PHP 8.3 + pcov), PHPCS/PHP-CS-Fixer-Läufe reaktiviert; Abstimmung mit QA bzgl. fehlender lokaler Composer-Installation offen. Referenz: [Backlog B9](README.md#backlog-b9). |
...
| PHPCS | `composer phpcs` | ✅ Aktiv | CI prüft wieder automatisch auf PSR-12-Konformität; lokale Läufe vor Commits weiterhin empfohlen. |
| PHP-CS-Fixer (Dry Run) | `vendor/bin/php-cs-fixer fix --dry-run --diff` | ✅ Aktiv | Dry-Run ist in CI reaktiviert und dient als zusätzliche Absicherung gegen Formatierungs-Regressionen. |
