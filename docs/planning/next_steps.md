...
- 🚧 **Quality gates & CI (B9) underway**: CI-Workflow erstellt (`.github/workflows/ci.yml`) und bindet PHPUnit (mit Coverage-Limits), PHPStan, PHPCS, PHP-CS-Fixer (dry-run) sowie `composer audit` ein; PHPStan und der PHP-CS-Fixer-Dry-Run laufen nun wieder regulär in der Pipeline. Lokale Verifikation bleibt blockiert, bis Composer-Abhängigkeiten installiert werden können. Coverage-Grenzen (80 % Lines / 75 % Branches) werden nun per `tools/coverage-guard.php` automatisiert geprüft.
