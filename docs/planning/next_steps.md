...
- 🚧 **Quality gates & CI (B9) underway**: CI-Workflow erstellt (`.github/workflows/ci.yml`) und bindet PHPUnit (mit Coverage-Limits), PHPStan sowie `composer audit` ein; die PHPCS- und PHP-CS-Fixer-Schritte sind vorübergehend deaktiviert, bis die aktuellen Sniff-Regressionen behoben wurden. Lokale Verifikation bleibt blockiert, bis Composer-Abhängigkeiten installiert werden können. Coverage-Grenzen (80 % Lines / 75 % Branches) werden weiterhin per `tools/coverage-guard.php` automatisiert geprüft.
